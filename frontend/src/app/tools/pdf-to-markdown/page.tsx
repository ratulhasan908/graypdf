...
export default function PdfToMarkdownPage() {
    ...
    async function handleConvert() {
        ...
        try {
            ...
            const result = await apiUpload<Job>(
        ...
        >
            {!job && (
                <>
                            ...
                            <label className="flex items-start gap-3 cursor-pointer">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-[#0D1C42]/60">
                                        Keeps headings, lists, tables, and links as Markdown.
                                    </p>
                                </div>
                            </label>
                            <label className="flex items-start gap-3 cursor-pointer">
                                ...
                                <div>
                                    <p className="font-medium text-sm text-[#010736]">
                                        Simple (plain text per page)
                                    </p>
                                    <p className="text-xs text-[#0D1C42]/60">
                                        Raw text with page separators. Best when structure isn&apos;t
                                        important.
                                    </p>
                                </div>
                            </label>
                    ...
                    >
                        {loading ? "Uploading..." : "Convert to Markdown"}
                    </button>
                </>
            )}
            ...
            {job && (job.status === "pending" || job.status === "processing") && (
                <div className="card p-12 text-center animate-fade-in">
                    ...
                    <h2 className="text-xl font-bold mb-2 text-[#010736]">
                        {job.status === "pending" ? "Queued..." : "Extracting..."}
                    </h2>
            ...
            {job && job.status === "completed" && job.download_url && (
                <div className="card p-8 text-center animate-scale-in">
                    ...
                    <p className="text-[#0D1C42]/60 mb-6">
                        Your Markdown file is ready. Open it in any editor or with an LLM.
                    </p>
                    <a
